...
interface CleanerProfileProps {
    ...
    onMessage: (cleaner: Cleaner) => void;
}

const CleanerProfile: React.FC<CleanerProfileProps> = ({ cleaner, onNavigate, onBook, onMessage }) => {
    return (
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg">
            ...
            <p className="mt-4 max-w-2xl mx-auto text-center">{cleaner.bio}</p>
             <div className="flex justify-center mt-8 gap-4">
                 <button 
                    onClick={() => onMessage(cleaner)}
                    className="flex items-center gap-2 bg-white text-primary border border-primary px-6 py-3 rounded-lg font-bold hover:bg-green-50"
                >
...
const App: React.FC = () => {
    ...
    const renderContent = () => {
        ...
        switch (view) {
            ...
            case 'cleanerProfile':
                if (selectedCleaner) {
                    return <CleanerProfile cleaner={selectedCleaner} onNavigate={handleNavigate} onBook={handleStartBookingProcess} onMessage={handleMessageCleaner}/>;
                }
                handleNavigate('landing');
                return null;
            case 'subscription':
