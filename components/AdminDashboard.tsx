...
import { User, Booking, Receipt, AdminRole } from '../types';
import { UserGroupIcon, StarIcon, XCircleIcon, EyeIcon } from './icons';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ allUsers, onUpdateUser, onDeleteUser, onMarkAsPaid, onConfirmPayment, onApproveSubscription }) => {
    ...
    const [activeTab, setActiveTab] = useState<'clients' | 'cleaners' | 'payments' | 'confirmations' | 'allBookings' | 'admins'>('clients');
    ...
    useEffect(() => {
        const fetchMe = async () => {
            try {
                ...
                if (me.adminRole === 'Payment') setActiveTab('payments');
                else if (me.adminRole === 'Verification') setActiveTab('confirmations');
                else setActiveTab('clients');

            } catch (error) {
                console.error("Failed to fetch current admin details");
            }
        };
        fetchMe();
    }, []);
    ...
    const UserTable: React.FC<UserTableProps> = ({ users, onView, onSuspend, onDelete, isAdminTable }) => {
        const isCleanerTable = !isAdminTable && users[0]?.role === 'cleaner';
        return (
    ...
    // Only render dashboard if we have current user context
                ...
                {activeTab === 'payments' && canSeePayments && (
                 ...
                 {activeTab === 'confirmations' && canSeeConfirmations && (
                    <div>
                        ...
                        {(currentUser.adminRole === 'Super' || currentUser.adminRole === 'Payment') && (
                        ...
                        {(currentUser.adminRole === 'Super' || currentUser.adminRole === 'Verification') && (
                            <div className="mt-4">
